...
class NetworkFilter {
    ...
    static parse(line, debug = false) {
            ...
            for (let i = 0; i < options.length; i += 1) {
                ...
                switch (option) {
                    ...
                    case 'first-party':
                        ...
                        break;
                    case 'fuzzy':
                        mask = utils_1.setBit(mask, 65536 /* fuzzyMatch */);
                        break;
                    case 'redirect-rule':
                    case 'redirect':
        ...
        if (filterIndexEnd - filterIndexStart >= 2 &&
            ...
            if (hostname !== undefined) {
                if (utils_1.getBit(mask, 268435456 /* isHostnameAnchor */) &&
                    utils_1.fastStartsWith(hostname, 'www.')) {
        ...
        if (this.fuzzySignature === undefined) {
            this.fuzzySignature =
                this.filter !== undefined && this.isFuzzy()
                    ? utils_1.createFuzzySignature(this.filter)
                    : EMPTY_ARRAY;
        }
        ...
        if (tokens_buffer_1.TOKENS_BUFFER.empty() === true && this.fromAny() === false) {
            ...
            if (types.length !== 0) {
                ...
                for (let i = 0; i < types.length; i += 1) {
                    const arr = new Uint32Array(1);
                    arr[0] = request_1.NORMALIZED_TYPE_TOKEN[types[i]];
                    result.push(arr);
                }
                return result;
            }
    ...
    isFuzzy() {
        return utils_1.getBit(this.mask, 65536 /* fuzzyMatch */);
    }
    isException() {
