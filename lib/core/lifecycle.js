...
class Lifecycle {
    ...
    async mainQueue() {
        ...
        socket.emit('log', 'Браузер запущен')
    ...
    async run() {
        ...
        socket.emit('log', `Начинаю обработку поискового запроса: ${this.singleQueryData.query} (${this.singleQueryData.pages} страниц(ы))`)
        const userAgent = config.common.userAgents[getRandom(0, config.common.userAgents.length)];
        socket.emit('log', userAgent)
        log(userAgent)
        ...
        if (!resNum || resNum === 0) {
            socket.emit('log', 'Ошибка при обработке запроса, пробую еще раз');
            isGo = true;
            ...
            socket.emit('log', `Обработка запроса заняла ${queryTime} секунд! Всего получено ${resNum} новостей. Всего обработано ${queryCount} запросов за ${secToMinToHoursConvert(fullTimeWorking)}!`);
    ...
    async stop() {
        isServer = false;
        isGo = true;
        socket.emit('log', 'Останавливаю бота')
        process.exit(0);
    }
}
