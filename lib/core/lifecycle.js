...
export default class LifeCycle {
            ...
            if (isGo && isServer && !isSingle) {
                ...
                for (let i = 0; i < Object.keys(engines).length; i++) {
                    if (Object.values(engines)[i] === true) {
                        ...
                        let id_engine = Number(Object.keys(engines)[i])
                        ...
                        store.setPage(page)
                        await this.run(page, id_request, id_engine)
                        await page.close()
                    }
                }
                isGo = true
    ...
    async run(page, id_request, id_engine) {
        let now = new Date();
        let goTime = new Date();
        let queryCount = 0;
        let AllNews = 0;
        ...
        const router = new Router(page, query, id_request, id_engine)
