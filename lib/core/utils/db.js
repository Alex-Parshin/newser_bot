        ...
        try {
            await new Promise(async(resolve, reject) => {
                await amqp.connect(`amqp://admin:admin@10.19.19.4:5672`, function(error0, connection) {
                    // await amqp.connect(`amqp://admin:admin@localhost:5672`, function(error0, connection) {
                    if (error0) {
                        reject(error0)
                        return
                    }
                    console.log(clc.green("[X] Connected to RabbitMQ"))
                    connection.createChannel(function(error1, channel) {
                        ...
                        channel.prefetch(1);
                        console.log(clc.green("[X] Channel created"))
                        ...
                        console.log(clc.green("[X] Data sent"))
                        resolve();
