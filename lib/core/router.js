 ...
 * @param {object} browser - объект класса Browser, открытый браузер
...
class Router {
    ...
    constructor(browser, page, query, id_request, pages, configuration, id_engine, socket = {}) {
        ...
        try {
            switch (Number(this.id_engine)) {
                ...
                case 7:
                    ...
                    break
                case 3:
                    News_array = await new Google(this.page, this.pages, this.query, this.configuration, this.id_request, this.id_engine, this.socket).index()
                    await this.page.close()
                    break
                default:
