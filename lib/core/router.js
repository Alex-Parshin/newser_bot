...
export default class Router {
    ...
    constructor(page, query, id_request, id_engine) {
        this.page = page
        this.query = query
    ...
    async toEngine() {
        ...
        try {
            switch (Number(store.getIdEngine())) {
                case 4: News_array = await new Yandex(this.page, this.query, this.id_request, this.id_engine).index()
                    break
                case 7: News_array = await new Rambler(this.page, this.query, this.id_request, this.id_engine).index()
                    break
                case 3: News_array = await new Google(this.page, this.query, this.id_request, this.id_engine).getRSS()
                    break
                default:
