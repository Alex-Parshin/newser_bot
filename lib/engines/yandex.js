...
export default class Yandex extends WSCN {
    ...
    constructor(singleQueryData) {
        super(singleQueryData, store.getConfig().engines.yandex)
    ...
    async index() {
        const relNewsFunc = async() => {
            var news_set = [];
            var item_hrefs = await this.singleQueryData.page.$$(this.configuration.selectors.relatedNews);
            console.log(`Похожих новостей ${item_hrefs.length}`)
            for (let i = 0; i < item_hrefs.length; i++) {
                var item_hrefs_href = await this.singleQueryData.page.evaluate(el => el.getAttribute('href'), item_hrefs[i]);
